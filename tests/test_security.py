...
from ae2.security.models import (
    User, Role, Permission, TokenData, LoginRequest, LoginResponse,
    SecurityConfig, create_default_user, get_role_permissions
)
from ae2.security.auth import AuthManager, get_current_user, require_permission
...
class TestAuthManager:
    ...
    def test_account_lockout(self):
        ...
        # Create a test user
        user = self.auth_manager.create_user(username, "test@example.com", Role.VIEWER, "password123")
    ...
    def test_user_management(self):
        ...
        # Create user
        user = self.auth_manager.create_user("newuser", "new@example.com", Role.OPERATOR, "password123")
    ...
    def test_duplicate_user_creation(self):
        ...
        self.auth_manager.create_user("testuser", "test@example.com", Role.VIEWER, "password123")
        
        # Try to create user with same username
        with pytest.raises(ValueError, match="Username testuser already exists"):
            self.auth_manager.create_user("testuser", "test2@example.com", Role.VIEWER, "password123")
        
        # Try to create user with same email
        with pytest.raises(ValueError, match="Email test@example.com already exists"):
            self.auth_manager.create_user("testuser2", "test@example.com", Role.VIEWER, "password123")
