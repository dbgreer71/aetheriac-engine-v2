# tests/test_basic.py
from ae2.contracts.models import Query, QueryType
...
class TestRouter:
    def test_route_simple(self):
        ...
        q = Query(text="what is ospf", query_type=QueryType.DEFINITION)
...
class TestIntegration:
    def test_dns_term_present(self):
        router = DefinitionalRouter()
        q = Query(text="explain dns", query_type=QueryType.DEFINITION)
        route = router.route_query(q)
