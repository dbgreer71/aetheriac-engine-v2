...
class TestContracts:
    ...
    def test_concept_card_creation(self):
        ...
        card = ConceptCard(
            id="concept:arp:v1", definition=definition, built_at=datetime.utcnow()
        )
...
class TestRouter:
    """Test unified router functionality."""

    def setup_method(self):
        """Setup router for each test."""
        # Mock stores for testing
        self.stores = {"index_store": None, "concept_store": None}

    def test_definition_query_classification(self):
        """Test classification of definition queries."""
        decision = route("What is ARP?", self.stores)

        assert decision.intent == "DEFINE"
        assert decision.confidence > 0.3
        assert decision.target == "826"  # ARP RFC
        assert "arp" in decision.matches

    def test_concept_query_classification(self):
        """Test classification of concept queries."""
        decision = route("Compare ARP and DNS", self.stores)

        assert decision.intent in ["CONCEPT", "DEFINE"]  # May fall back to DEFINE
        assert decision.confidence > 0.3
        assert "arp" in decision.matches or "dns" in decision.matches

    def test_troubleshooting_query_classification(self):
        """Test classification of troubleshooting queries."""
        decision = route("OSPF neighbor down", self.stores)

        assert decision.intent == "TROUBLESHOOT"
        assert decision.confidence > 0.3
        assert "ospf" in decision.matches
        assert any("neighbor" in match for match in decision.matches)

    def test_router_decision_structure(self):
        """Test router decision structure."""
        decision = route("What is TCP?", self.stores)

        assert hasattr(decision, "intent")
        assert hasattr(decision, "target")
        assert hasattr(decision, "confidence")
        assert hasattr(decision, "matches")
        assert hasattr(decision, "notes")
        assert hasattr(decision, "mode_used")
        assert isinstance(decision.intent, str)
        assert isinstance(decision.confidence, float)
        assert isinstance(decision.matches, list)
...
class TestAssembler:
    ...
    def test_contains_definition(self):
        ...
        definition_text = (
            "ARP is defined as a protocol for mapping IP addresses to MAC addresses."
        )
        non_definition_text = "This is just some random text about networking."

        assert self.assembler._contains_definition(definition_text) is True
        assert self.assembler._contains_definition(non_definition_text) is False
    ...
    def test_high_quality_definition_detection(self):
        ...
        assert self.assembler._is_high_quality_definition(section) is True
...
class TestSettings:
    ...
    def test_settings_initialization(self):
        ...
        assert settings.enable_rfc is True
        assert settings.strict_definitions is True
...
class TestIntegration:
    ...
    def test_router_to_assembler_flow(self):
        ...
        # Create a test query
        query = Query(text="What is ARP?", query_type=QueryType.DEFINITION)

        # Route the query using new router
        stores = {"index_store": None, "concept_store": None}
        decision = route(query.text, stores)
        ...
        response = assembler.assemble_definition(
            query=query, retrieved_sections=retrieved_sections, query_id=query_id
        )
