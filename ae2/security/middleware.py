...
class SecurityMiddleware(BaseHTTPMiddleware):
    ...
    async def _cleanup_rate_limit_data(self) -> None:
        ...
        if cleaned_clients:
            logger.debug(f"Cleaned up rate limit data for {len(cleaned_clients)} clients")
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        for param_name, param_value in request.query_params.items():
            if self._contains_forbidden_content(param_value):
                logger.warning(f"Forbidden content detected in query parameter: {param_name}")
                return JSONResponse(
                    status_code=400,
                    content={
                        "error": "invalid_input",
...
class AuditMiddleware(BaseHTTPMiddleware):
    ...
    def _log_security_event(
        ...
        is_security_event = (
            ...
            "authorization" in request.headers.get("authorization", "").lower()
        )
