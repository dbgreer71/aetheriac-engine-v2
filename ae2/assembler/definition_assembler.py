...
class DefinitionAssembler:
    ...
    def __init__(self):
        ...
        self.logger.info(
            f"Definition assembler initialized (strict_mode: {self.strict_mode})"
        )
    ...
    def assemble_definition(
        ...
        if not definition_sections and self.strict_mode:
            return self._create_strict_mode_failure_response(
                query, query_id, start_time
            )

        # Select the best definition section
        best_section, confidence = self._select_best_definition(
            definition_sections or retrieved_sections
        )
    ...
    def _filter_definition_sections(
        self, sections: List[Tuple[RFCSection, float]]
        ...
        self.logger.debug(
            f"Filtered {len(definition_sections)} definition sections from {len(sections)} total"
        )
    ...
    def _construct_definition_content(
        self, section: RFCSection, confidence: float
    ) -> Dict[str, any]:
    ...
    def _extract_citations(self, sections: List[RFCSection]) -> List[str]:
        ...
        for section in sections:
            citation = (
                f"RFC {section.rfc_number}, Section {section.section}: {section.title}"
            )
    ...
    def _create_empty_response(
        self, query: Query, query_id: str, start_time: float
    ) -> QueryResponse:
