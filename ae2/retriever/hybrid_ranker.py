...
class HybridRanker:
    ...
    def __init__(self):
        ...
        self.logger.info(
            f"Initialized hybrid ranker with model: {settings.embedding_model}"
        )
    ...
    def build_index(self, documents: List[RFCSection]) -> None:
        ...
        self.bm25_index = BM25Okapi(
            tokenized_texts, k1=settings.bm25_k1, b=settings.bm25_b
        )
    ...
    def search(
        self, query: str, top_k: int = 10, dense_weight: Optional[float] = None
        ...
        self.logger.debug(
            f"Hybrid search returned {len(results)} results for query: {query}"
        )
        return results
