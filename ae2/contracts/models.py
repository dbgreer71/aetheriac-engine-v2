...
class RFCSection(BaseModel):
    ...
    built_at: datetime = Field(default_factory=datetime.utcnow, description="When this section was built")
...
class ConceptCard(BaseModel):
    ...
    id: str = Field(..., description="Unique concept identifier (e.g., 'concept:arp:v1')")
    definition: Definition = Field(..., description="Primary definition")
    claims: List[Claim] = Field(default_factory=list, description="Claims about the concept")
    provenance: Dict[str, Any] = Field(default_factory=dict, description="Provenance information")
    built_at: datetime = Field(default_factory=datetime.utcnow, description="When this card was built")
...
class Artifact(BaseModel):
    ...
    artifact_type: str = Field(..., description="Type of artifact (config, log, capture, etc.)")
...
class Case(BaseModel):
    ...
    id: str = Field(default_factory=lambda: str(uuid4()), description="Unique case identifier")
    ...
    artifacts: List[Artifact] = Field(default_factory=list, description="Associated artifacts")
    provenance: Dict[str, Any] = Field(default_factory=dict, description="Provenance information")
    created_at: datetime = Field(default_factory=datetime.utcnow, description="When this case was created")
...
class PlaybookRule(BaseModel):
    ...
    citations: List[str] = Field(default_factory=list, description="Citations to RFC sections or cases")
...
class Playbook(BaseModel):
    ...
    applies_to: List[str] = Field(..., description="Protocols/technologies this applies to")
    rules: List[PlaybookRule] = Field(..., description="Rules in this playbook")
    created_at: datetime = Field(default_factory=datetime.utcnow, description="When this playbook was created")
