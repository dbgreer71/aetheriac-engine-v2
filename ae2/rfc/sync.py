...
class RFCSectionizer:
    ...
    def sectionize_xml(self, xml_content: str, rfc_number: int) -> List[RFCSection]:
        ...
        try:
            ...
            rfc_title = title_elem.text if title_elem is not None else f"RFC {rfc_number}"
            ...
            for section in root.findall('.//rfc/middle/section'):
                ...
                section_title = title_elem.text if title_elem is not None else f"Section {section_number}"
                ...
                if excerpt:
                    # Generate hash
                    normalized_text = f"{rfc_number}:{section_number}:{excerpt}".lower().strip()
                    content_hash = hashlib.sha256(normalized_text.encode('utf-8')).hexdigest()
                    
                    # Build URL
                    url = urljoin(settings.rfc_mirror_url, f"rfc{rfc_number}.xml#{section_id}")
            ...
            self.logger.info(f"Sectionized RFC {rfc_number} into {len(sections)} sections")
            return sections
    ...
    def sectionize_txt(self, txt_content: str, rfc_number: int) -> List[RFCSection]:
        ...
        for line in lines:
            ...
            if line and line[0].isdigit() and '.' in line[:10]:
                ...
                if current_section and current_content:
                    ...
                    if excerpt:
                        normalized_text = f"{rfc_number}:{current_section}:{excerpt}".lower().strip()
                        content_hash = hashlib.sha256(normalized_text.encode('utf-8')).hexdigest()
        ...
        self.logger.info(f"Sectionized RFC {rfc_number} (text) into {len(sections)} sections")
        return sections
...
class RFCSyncer:
    ...
    async def download_rfc(self, rfc_number: int) -> Tuple[Optional[str], Optional[str]]:
    ...
    async def sync_rfc(self, rfc_number: int) -> List[RFCSection]:
        ...
        if xml_content:
            try:
                ...
                self.logger.warning(f"XML sectionization failed for RFC {rfc_number}, trying TXT")
